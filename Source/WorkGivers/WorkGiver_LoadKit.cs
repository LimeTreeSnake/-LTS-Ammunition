...
namespace Ammunition.WorkGivers {
    public class WorkGiver_LoadKit : WorkGiver_Scanner {
        ...
        public override Job JobOnThing(Pawn pawn, Thing t, bool forced = false) {
            Job job = JobMaker.MakeJob(Defs.JobDefOf.LTS_FetchAmmo, t, Logic.AmmoLogic.GetWornKits(pawn)?.FirstOrDefault(x =>  x.KitComp.Bags.Where(y => y.ChosenAmmo.defName == t.def.defName && y.Count < y.MaxCount).Any()));
